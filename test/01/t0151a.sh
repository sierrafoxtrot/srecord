...
#
$bin/test_crc16 -x < zero.length.file > test.out
test $? -eq 0 || fail
echo >> test.out
test $? -eq 0 || no_result

$bin/test_crc16 -x < single.a.file >> test.out
test $? -eq 0 || fail
echo >> test.out
test $? -eq 0 || no_result

$bin/test_crc16 -x < nine.digits.file >> test.out
test $? -eq 0 || fail
echo >> test.out
test $? -eq 0 || no_result

$bin/test_crc16 -x < upper-case-a.256.file >> test.out
test $? -eq 0 || fail
