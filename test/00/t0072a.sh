...
#
# MANIFEST: Test the EMON52 functionality
#
here=`pwd`
...
fail()
{
        cd $here
        rm -rf $work
        echo 'FAILED test of the EMON52 functionality'
        exit 1
}
...
no_result()
{
        cd $here
        rm -rf $work
        echo 'NO RESULT for test of the EMON52 functionality'
        exit 2
}
...
$bin/srec_cat test.in -emon52 -o test.out -header HDR
...
$bin/srec_cat test.in -emon52 -o test.out -emon52
if test $? -ne 0; then fail; fi

diff test.ok test.out
if test $? -ne 0; then fail; fi

$bin/srec_cmp test.in -emon52 test.out -emon52
if test $? -ne 0; then fail; fi
