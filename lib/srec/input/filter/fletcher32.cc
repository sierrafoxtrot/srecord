...
srec_input_filter_fletcher32::~srec_input_filter_fletcher32()
{
}
...
srec_input::pointer
srec_input_filter_fletcher32::create(const srec_input::pointer &a_deeper,
    unsigned long a_address, endian_t a_end)
{
    return
        pointer(new srec_input_filter_fletcher32(a_deeper, a_address, a_end));
}


bool
srec_input_filter_fletcher32::read(srec_record &record)
{
    //
    // If we haven't read the deeper input yet, read all of it into
    // a memory buffer, then fletcher32 the bytes.
    //
    if (buffer.empty())
    {
        ...
        if (buffer.has_holes())
        {
            warning
            (
                "The data presented for Fletcher-32 checksum has at "
                ...
                "--fill 0xFF filter *before* this Fletcher-32 filter to "
